...
(function($){
    ...
    var settings = $.extend({
			...
			initButtonText: 		'Technical Issues',
			strokeStyle:			'black',
			shadowColor:			'black',
			shadowOffsetX:			1,
			shadowOffsetY:			1,
			...
			tpl: {
				description:	'<div id="feedback-welcome"><div class="feedback-logo">Feedback</div><p>This is the best way to tell us about issues within the app. We welcome headsup regarding language, images, feature ideas and general comments.</p><p>Start by writing a brief description of the problem:</p><textarea id="feedback-note-tmp"></textarea><p>If relevant, you can identify areas of the page related to the issue.</p><button id="feedback-welcome-next" class="feedback-next-btn feedback-btn-gray">Next</button><div id="feedback-welcome-error">Please enter a description.</div><div class="feedback-wizard-close"></div></div>',
		...
		if (supportedBrowser) {
			if(isFeedbackButtonNative) {
				$('body').append('<button id = "technicalEl" class="feedback-btn feedback-btn-gray">' + settings.initButtonText + '</button>');
			}
			$(document).on('click', settings.feedbackButton, function(){
				...
				tpl += settings.tpl.highlighter + settings.tpl.overview + '<canvas id="feedback-canvas" style="width:' + w + 'px;height:' + h + 'px"></canvas><div id="feedback-helpers"></div><input id="feedback-note" name="feedback-note" type="hidden"></div>';
